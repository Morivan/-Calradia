...
class Command(BaseCommand):
    help = "Export workshop data to CSV files in exports/yandex-tables/"
    ...
    def _export_clients(self, out_dir: Path) -> None:
        ...
        self._write(out_dir / "1_Клиенты.csv", headers, rows)
    ...
    def _export_orders(self, out_dir: Path) -> None:
        ...
        self._write(out_dir / "2_Заказы.csv", headers, rows)
    ...
    def _export_materials(self, out_dir: Path) -> None:
        ...
        rows = [
            [m.id, m.name, m.type, m.direction, m.unit,
             m.price or "", m.stock, m.min_stock, m.supplier, m.notes]
            for m in Material.objects.order_by("direction", "name")
        ]
        self._write(out_dir / "3_Материалы.csv", headers, rows)
    ...
    def _export_colleagues(self, out_dir: Path) -> None:
        ...
        self._write(out_dir / "6_Коллеги.csv", headers, rows)
    ...
    def _export_products(self, out_dir: Path) -> None:
        ...
        self._write(out_dir / "7_Каталог.csv", headers, rows)
